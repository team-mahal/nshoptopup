<template>
    <div class="container mx-auto w-full md:w-6/12 xl:w-6/12 p-2 md:p-0" style="min-height: 460px">
        <div class="rounded-full h-24 w-24 m-auto flex items-center justify-center bg-red-300 mt-2 lg:mt-8 text-white text-2xl md:text-4xl font-bold" id="prodile">
            {{ nameFirstChar() }}
        </div>
        <h1 class="text-center text-2xl lg:text-4xl font-extrabold">{{ user.name }}</h1>
        <h4 class="text-center ">User ID : <b>{{ user.id }}</b></h4>
        <h4 class="text-center  mb-2 lg:mb-6">Email : <b>{{ user.email }}</b></h4>
