		...
		<div class="card mt-5">
			<div class="card-header border-0">
				<div class="row align-items-center">
					<div class="col">
						<h3 class="mb-0">Deposite config List</h3>
					</div>
					<div class="col text-right">
						<a href="{{ route('depositeconfig.create') }}" class="btn btn-sm btn-primary">Create New</a>
					</div>
				</div>
			</div>
			<?php if(empty($datas)){  ?>
			...
			<div class="table-responsive">
				...
				<table class="table align-items-center table-flush">
					<thead class="thead-light">
						<tr>
							<th scope="col">#</th>
							<th scope="col">Fee Amount <sub>(%)</sub></th>
					...
					<tbody>
						...
						<tr>
							...
							<td>{{ $data->percentage_fee }}</td>
							<td style="display: -webkit-inline-box;">
								<a href="{{ route('depositeconfig.edit',[$data->id]) }}" class="btn btn-success btn-sm">Edit</a>
								<!-- <a href="productDelete/{{ $data->id }}" class="btn btn-danger btn-sm">Delete</a> -->
								<form action="{{ route('depositeconfig.destroy',$data->id) }}" method="POST">
									@csrf
                    				@method('DELETE')
